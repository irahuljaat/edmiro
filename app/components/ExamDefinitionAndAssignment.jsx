...
function ExamDefinitionAndAssignment({ activeSession, schoolId = 'TEST_EDMIRO_ACADEMY' }) {
    ...
    const fetchExams = useCallback(async () => {
        ...
        try {
            const snapshot = await getDocs(collection(db, 'Data', schoolId, 'sessions', activeSession, 'exams'));
    ...
    const fetchCurrentAssignments = useCallback(async (examId, classes) => {
        ...
        setLoading(true);
        try {
            // 1. Fetch subjects assigned to the first selected class from Data > schoolId > sessions > activeSession > subjects > classId
            const firstClass = classes[0];
            const classSubjectRef = doc(db, 'Data', schoolId, 'sessions', activeSession, 'subjects', firstClass);
            ...
            // 2. Fetch existing exam assignments from Data > schoolId > sessions > activeSession > examAssignments > docId
            const docId = `${examId}_${firstClass}`;
            const docRef = doc(db, 'Data', schoolId, 'sessions', activeSession, 'examAssignments', docId);
    ...
    const handleSaveExamName = async () => {
        if (!examNameInput.trim() || !activeSession || !schoolId) return;
        setLoading(true);
        const name = examNameInput.trim();
        const examId = name.toLowerCase().replace(/\s+/g, '-');
        try {
            await setDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'exams', examId), { 
    ...
    const handleAssignSubjects = async () => {
        ...
        if (!selectedExamId || selectedClasses.length === 0 || selectedSubjects.length === 0 || !activeSession || !schoolId) {
        ...
        try {
            ...
            const promises = selectedClasses.map(classId => {
                ...
                return setDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'examAssignments', docId), {
                        ...
                        <div className="bg-white rounded-[28px] p-6 md:p-8 shadow-sm border border-slate-100">
                            ...
                            <div className="space-y-3">
                                ...
                                <button 
                                    ...
                                    className="w-full text-white py-3 rounded-full font-bold text-xs uppercase tracking-widest shadow-md transition-all active:scale-[0.99] cursor-pointer"
                                ...
                                <div className="grid grid-cols-4 gap-2 h-48 overflow-y-auto pr-2 custom-scrollbar">
                                    {MOCK_CLASSES.map(c => (
                                        <button
                                            ...
                                            className={`py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border cursor-pointer ${
                        ...
                        <div className="bg-white rounded-[28px] p-6 md:p-8 shadow-sm border border-slate-100">
                            <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-100">
                                ...
                                <button
                                    ...
                                    disabled={loading}
                                    className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-full font-bold text-xs uppercase tracking-widest shadow-md hover:bg-slate-800 transition-colors disabled:opacity-50 cursor-pointer"
                                >
