...
const fetchTeachers = async (schoolId) => {
    ...
    const teachersCollection = collection(db, 'Data', schoolId, 'teachers');
    const teacherSnapshot = await getDocs(teachersCollection); 
    return teacherSnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
};

const deleteTeacherFromDb = async (schoolId, id) => {
    if (!schoolId || !id) return;
    // Delete teacher from Data -> {schoolId} -> teachers -> {id}
    await deleteDoc(doc(db, 'Data', schoolId, 'teachers', id));
};

export default function TeacherPageController({ schoolId, activeSession, setCurrentView: parentSetCurrentView, setSelectedTeacher: parentSetSelectedTeacher, searchTerm: externalSearchTerm }) {
    ...
    const handleAction = (view, teacher) => {
        if (parentSetCurrentView && parentSetSelectedTeacher) {
            parentSetSelectedTeacher(teacher);
            parentSetCurrentView(view);
        } else {
    ...
    const handleBackToList = () => {
        if (parentSetCurrentView) {
            parentSetCurrentView('LIST');
        } else {
    ...
    const handleDelete = async (id, name) => {
        ...
        try {
            await deleteTeacherFromDb(schoolId, id);
    ...
    // Render Edit Form View if controlled internally
    if (currentView === 'EDIT') {
        return (
            <EditTeacherForm 
                schoolId={schoolId}
    ...
    // Render View / Print View if controlled internally
    if (currentView === 'VIEW') {
        return (
            <TeacherViewPrint 
                schoolId={schoolId}
                                            ...
                                            <tr key={teacher.id} className="group hover:scale-[1.001] transition-all">
                                                ...
                                                <td className="px-6 py-4 rounded-r-[1.5rem] backdrop-blur-sm border-y border-r border-slate-100 shadow-sm text-right"
                                                    ...
                                                    <div className="flex items-center justify-end gap-2">
                                                        {/* View Profile & Print */}
                                                        <button 
                                                            onClick={() => handleAction('VIEW_PRINT', teacher)} 
