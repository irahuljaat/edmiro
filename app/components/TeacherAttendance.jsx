...
export default function TeacherAttendance({ schoolId }) {
    ...
    // --- FIRESTORE LOGIC ---
    const fetchTeacherList = async (targetSchoolId) => {
        if (!targetSchoolId) return [];
        // Read teachers from Data -> {schoolId} -> teachers
        const q = query(collection(db, 'Data', targetSchoolId, 'teachers')); 
    ...
    const fetchAttendanceByDate = async (targetSchoolId, date) => {
        if (!targetSchoolId) return {};
        // Read attendance records from Data -> {schoolId} -> teacherAttendance -> {date} -> records
        const recordsCollectionRef = collection(doc(db, 'Data', targetSchoolId, 'teacherAttendance', date), 'records');
    ...
    // Constant 2-element dependency array
    useEffect(() => {
        let isMounted = true;
        ...
        const loadInitialData = async () => {
            ...
            try {
                setLoading(true);
                const list = await fetchTeacherList(schoolId);
                const records = await fetchAttendanceByDate(schoolId, selectedDate);
    ...
    }, [selectedDate, schoolId]); // Always keeps 2 items in array
    ...
    const handleSaveAttendance = async () => {
        if (!schoolId) {
            setMessage({ type: 'error', text: 'School context missing.' });
            return;
        }
        ...
        try {
            ...
            // Save records under Data -> {schoolId} -> teacherAttendance -> {selectedDate} -> records -> {id}
            await Promise.all(recordsToSave.map(([id, record]) => {
                const cleaned = cleanDataForFirestore(record);
                return setDoc(doc(db, 'Data', schoolId, 'teacherAttendance', selectedDate, 'records', id), cleaned, { merge: true });
            }));
