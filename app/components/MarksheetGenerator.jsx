...
function MarksheetGenerator({ onBack, activeSession: propActiveSession, schoolId = 'TEST_EDMIRO_ACADEMY' }) {
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(doc(db, 'Data', schoolId, 'config', 'settings'), (docSnap) => {
            if (docSnap.exists()) {
                ...
                setActiveSession(data.activeSession || data.session || propActiveSession || '');
            }
        });
        return () => unsub();
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            let sSnap = await getDocs(query(collection(db, 'Data', schoolId, 'sessions', activeSession, 'students'), where('grade', '==', selectedClass)));
        ...
        <div className="min-h-screen bg-slate-50 p-4 md:p-8 print:p-0 print:bg-white">
            <div className="max-w-6xl mx-auto no-print space-y-6">
                <div className="flex justify-between items-center">
                    <button onClick={onBack} className="text-indigo-600 font-bold uppercase text-[10px] tracking-widest flex items-center gap-2 cursor-pointer">
                ...
                <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select value={selectedClass} onChange={e=>setSelectedClass(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl font-bold border-none ring-1 ring-slate-100 outline-none cursor-pointer">
                        ...
                        <div>
                            ...
                            <input type="date" value={resultDate} onChange={e => setResultDate(e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl font-bold ring-1 ring-slate-100 outline-none cursor-pointer" />
                        </div>
                        <div className="flex items-end gap-2">
                            <button onClick={fetchData} className="flex-1 bg-indigo-600 text-white p-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition-all cursor-pointer">
                                {loading ? 'Processing...' : 'Generate Marksheets'}
                            </button>
                            <button onClick={()=>window.print()} disabled={!printData.length} className="bg-slate-800 text-white p-3 rounded-xl disabled:opacity-50 cursor-pointer">
                                <HiPrinter size={20}/>
                            </button>
                        </div>
                    </div>
                ...
                {generatedData.length > 0 && (
                    <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                        <div className="p-4 bg-slate-50 border-b flex justify-between items-center">
                            ...
                            <button onClick={() => setSelectedStudentIds(selectedStudentIds.length === generatedData.length ? [] : generatedData.map(r => r.student.id))} className="text-[10px] font-bold text-indigo-600 uppercase cursor-pointer">
                                Toggle All
                            </button>
                        </div>
