...
function BulkAdmissionManager({ onComplete, activeSession, schoolId: propSchoolId }) {
    ...
    const handleFileUpload = (event) => {
        ...
        if (!currentSchoolId) {
            setMessage({ type: 'error', text: 'No school branch context detected.' });
            return;
        }
    ...
    const processBulkUpload = async (studentsArray) => {
        ...
        // Direct write to Data -> {schoolId} -> sessions -> {activeSession} -> students
        const sessionStudentsCollection = collection(db, 'Data', currentSchoolId, 'sessions', activeSession, 'students');
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex justify-center flex-wrap gap-3">
                ...
                {userRole === 'super' && availableBranches.length > 0 && (
                    <div className="inline-flex items-center gap-1.5 px-4 py-2 rounded-2xl border border-slate-200 bg-slate-50">
                        <HiOutlineOfficeBuilding className="text-slate-400 w-4 h-4" />
                        <select 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-slate-50 p-8 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center text-center group hover:border-indigo-400 transition-colors">
                    ...
                    <button onClick={handleDownloadTemplate} className="mt-6 px-8 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-black uppercase hover:bg-slate-900 hover:text-white transition-all shadow-sm cursor-pointer">
                        Download Template
                    </button>
                </div>
