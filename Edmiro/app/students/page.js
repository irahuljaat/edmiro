...
import { 
    ...
    HiOutlineTrash, HiOutlineUserGroup, HiOutlineOfficeBuilding 
} from 'react-icons/hi';
...
export default function StudentListPage() {
    ...
    // Initial Branch & User Context Setup
    useEffect(() => {
    ...
    // Fetch students under path: Data -> {schoolId} -> sessions -> {activeSession} -> students
    const fetchStudents = async () => {
        if (!activeSession || !currentSchoolId) return;
        setLoading(true);
        try {
            const snapshot = await getDocs(collection(db, 'Data', currentSchoolId, 'sessions', activeSession, 'students'));
    ...
    useEffect(() => { 
        if (activeSession && currentSchoolId) fetchStudents(); 
    }, [activeSession, currentSchoolId]);
    ...
    const handleDelete = async (studentId, studentName) => {
        if (window.confirm(`Permanently delete ${studentName}?`)) {
            try {
                await deleteDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'students', studentId)); 
                fetchStudents();
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {userRole === 'super' && availableBranches.length > 0 && (
                            <div className="flex items-center gap-2 bg-slate-50 px-4 py-2 rounded-full border border-slate-200">
                                <HiOutlineOfficeBuilding className="text-slate-400 text-base" />
                                <select 
