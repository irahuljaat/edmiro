...
export default function IdCardsExportPage({ schoolId: propSchoolId }) {
    ...
    // Initialize schoolId from localStorage or props
    useEffect(() => {
    ...
    // Fetch active session and students from Data -> {schoolId} -> ...
    useEffect(() => {
        const fetchSettingsAndStudents = async () => {
            if (!currentSchoolId) return;
            try {
                const settingsSnap = await getDoc(doc(db, "Data", currentSchoolId, "config", "settings"));
                if (settingsSnap.exists()) {
                    const session = settingsSnap.data().activeSession || settingsSnap.data().session || "2026-2027";
                    setActiveSession(session);

                    const studentsSnap = await getDocs(collection(db, "Data", currentSchoolId, "sessions", session, "students"));
    ...
    const handleExport = async () => {
        ...
        try {
            const studentsSnap = await getDocs(collection(db, "Data", currentSchoolId, "sessions", activeSession, "students"));
            ...
            for (let i = 0; i < students.length; i++) {
                ...
                const studentName = student.name || student.studentName || 'Unnamed';
                setStatusText(`Processing student ${i + 1} of ${students.length}: ${studentName}`);
    ...
    return (
        <div className="min-h-screen p-6 lg:p-8 font-sans" style={{ backgroundColor: colors.background || '#F4F6F8' }}>
            <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-[24px] shadow-sm border border-gray-100">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            <div className="p-3 text-slate-900 rounded-2xl shadow-sm" style={{ backgroundColor: primaryColor }}>
                ...
                <div className="bg-white p-8 rounded-[24px] shadow-sm border border-gray-100 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full bg-gray-50 border border-gray-200 text-gray-900 text-sm font-bold rounded-xl px-4 py-3 outline-none cursor-pointer"
                            >
                        ...
                        <div className="flex items-end pt-4 md:pt-0">
                            <button
                                ...
                                className="w-full text-slate-900 font-bold py-3.5 px-6 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
                    ...
                    {loading && (
                        <div className="p-6 rounded-2xl border border-amber-200 flex items-center gap-4 text-amber-900 bg-amber-50/80 animate-pulse">
