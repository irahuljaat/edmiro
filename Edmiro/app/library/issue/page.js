...
export default function LibraryMemberIssuePage() {
  ...
  // Fetch Active Session, Students, and Library Books from Firestore
  const fetchStudentsAndBooks = useCallback(async (targetSchoolId) => {
    if (!targetSchoolId || !hasMounted) return;
    
    setLoading(true);
    try {
      // 1. Fetch Settings & Active Session from Data -> {schoolId} -> config -> settings
      ...
      // 2. Fetch Students from Data -> {schoolId} -> sessions -> {activeSession} -> students
      const studentsCollectionRef = collection(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'students');
      ...
      const studentData = studentSnapshot.docs.map((docSnap, index) => {
        ...
        return {
          ...
          className: data.className || data.class || data.studentClass || data.grade || 'N/A',
      ...
      // 3. Fetch Books Catalog from Data -> {schoolId} -> libraryCatalog
      const booksColRef = collection(db, 'Data', targetSchoolId, 'libraryCatalog');
      const booksSnapshot = await getDocs(booksColRef);
      
      const booksList = booksSnapshot.docs.map(docSnap => {
        const data = docSnap.data();
        return {
          id: docSnap.id,
          title: data.bookTitle || data.title || 'Untitled Book',
          bookNumber: data.bookNumber || data.isbnNumber || 'N/A',
          qty: data.qty || 0,
      ...
      console.error('Error fetching student & library data:', err);
      setMembers([]);
      setAvailableBooks([]);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    fetchStudentsAndBooks(currentSchoolId);
  }, [currentSchoolId, fetchStudentsAndBooks]);
  ...
  const filteredMembers = members.filter(m => {
    ...
    const matchesClass = selectedClassFilter ? String(m.className) === String(selectedClassFilter) : true;
  ...
  const handleIssueBookSubmit = async (e) => {
    ...
    if (!currentSchoolId || !selectedMember) {
      alert('School or Member context missing.');
      return;
    }
    ...
    try {
      // Save issued book directly to Data -> {schoolId} -> sessions -> {activeSession} -> students -> {studentId}
      const studentDocRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'students', selectedMember.id);
  ...
  const handleReturnBook = async (studentId, issuedId) => {
    ...
    try {
      const studentDocRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'students', targetMember.id);
      await updateDoc(studentDocRef, {
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-0.5">
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                Active Session: <span className="font-bold" style={{ color: colors.primary }}>{activeSession}</span>
              </p>
