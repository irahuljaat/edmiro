...
import { HiSave, HiX, HiClock, HiCalendar, HiTrash, HiPrinter, HiBookOpen, HiPlus, HiCog, HiOfficeBuilding, HiCheck } from 'react-icons/hi';
...
export default function SchoolTimetableMatrixManager() {
    ...
    const [activeSession, setActiveSession] = useState('2024-25');
    const [schoolName, setSchoolName] = useState('Edmiro Test Academy');
    ...
    // --- BATCH SUBJECT MAPPING MODAL STATE ---
    const [isAssignSubjectsModalOpen, setIsAssignSubjectsModalOpen] = useState(false);
    const [selectedClassesForSubjects, setSelectedClassesForSubjects] = useState(['1']);
    ...
    const fetchSystemData = useCallback(async (targetSchoolId) => {
        if (!targetSchoolId) return;
        setIsLoading(true);
        try {
            let currentSession = '2024-25';
            
            // 1. Fetch settings from Data -> {schoolId} -> config -> settings
            try {
                const settingsDocRef = doc(db, 'Data', targetSchoolId, 'config', 'settings');
                const settingsSnap = await getDoc(settingsDocRef);
                if (settingsSnap.exists()) {
                    const data = settingsSnap.data();
                    if (data.activeSession || data.session) {
                        currentSession = data.activeSession || data.session;
                        setActiveSession(currentSession);
                    }
                }

                const schoolDetailsRef = doc(db, 'Data', targetSchoolId, 'config', 'schoolDetails');
                const detailsSnap = await getDoc(schoolDetailsRef);
                if (detailsSnap.exists()) {
                    const dData = detailsSnap.data();
                    if (dData.schoolName) setSchoolName(dData.schoolName);
                    if (dData.schoolLogo) setSchoolLogo(dData.schoolLogo);
                }
            } catch (err) {
                console.error("Error fetching system settings:", err);
            }

            // 2. Fetch teachers from Data -> {schoolId} -> teachers
            const teachersSnap = await getDocs(collection(db, 'Data', targetSchoolId, 'teachers'));
            const teacherList = teachersSnap.docs.map(docSnap => ({
                id: docSnap.id,
                name: docSnap.data().name || docSnap.data().teacherName || docSnap.id,
                employeeId: docSnap.data().employeeId || docSnap.id
            }));
            setTeachers(teacherList);

            // 3. Fetch Teacher Specific Timing Configurations
            const teacherConfigsSnap = await getDocs(collection(db, 'Data', targetSchoolId, 'teacherConfigs'));
            const fetchedConfigs = {};
            teacherConfigsSnap.docs.forEach(docSnap => {
                fetchedConfigs[docSnap.id] = docSnap.data();
            });

            const newConfigs = {};
            teacherList.forEach(t => {
                const savedConf = fetchedConfigs[t.id] || {};
                newConfigs[t.id] = {
                    inPeriod: savedConf.inPeriod || 1,
                    outPeriod: savedConf.outPeriod || 8,
                    periodInterval: savedConf.periodInterval || 45,
                    startTime: savedConf.startTime || '08:00 AM',
                    endTime: savedConf.endTime || '01:40 PM',
                    assignments: savedConf.assignments || []
                };
            });
            setTeacherConfigs(newConfigs);

            // 4. Fetch subjects from Data -> {schoolId} -> sessions -> {session} -> subjects
            const subjectsMap = {};
            for (const cls of classesList) {
                try {
                    const subDocRef = doc(db, 'Data', targetSchoolId, 'sessions', currentSession, 'subjects', cls);
            ...
            // 5. Fetch timetables from Data -> {schoolId} -> timetables
            const ttSnap = await getDocs(collection(db, 'Data', targetSchoolId, 'timetables'));
    ...
    useEffect(() => {
        fetchSystemData(currentSchoolId);
    }, [currentSchoolId, fetchSystemData]);
    ...
    useEffect(() => {
        initializeMatrix();
    }, [classesList, periodsConfig, initializeMatrix]);
    ...
    const updateTeacherConfig = async (teacherId, field, value) => {
        const updatedTeacherConfig = {
        ...
        setTeacherConfigs(prev => ({
            ...prev,
            [teacherId]: updatedTeacherConfig
        }));

        if (!currentSchoolId || !teacherId) return;

        try {
            const configRef = doc(db, 'Data', currentSchoolId, 'teacherConfigs', teacherId);
            await setDoc(configRef, updatedTeacherConfig, { merge: true });
        } catch (err) {
            console.error('Failed to save teacher timing settings:', err);
        }
    };
    ...
    const toggleSelectAllClasses = () => {
        if (selectedClasses.length === classesList.length) {
            setSelectedClasses([]);
        } else {
    ...
    const handleAddAssignment = async () => {
        if (selectedClasses.length === 0 || !assignSubject || !selectedConfigTeacher) return;

        const timestamp = Date.now();
        const createdAssignments = selectedClasses.map((clsName, idx) => ({
            className: clsName,
            subject: assignSubject,
            id: `${timestamp}_${idx}_${clsName}`
        }));
        
        const newAssignments = [
            ...(teacherConfigs[selectedConfigTeacher]?.assignments || []),
            ...createdAssignments
        ];

        const updatedConfig = {
            ...teacherConfigs[selectedConfigTeacher],
            assignments: newAssignments
        };

        setTeacherConfigs(prev => ({
            ...prev,
            [selectedConfigTeacher]: updatedConfig
        }));
        ...
        try {
            const configRef = doc(db, 'Data', currentSchoolId, 'teacherConfigs', selectedConfigTeacher);
            await setDoc(configRef, updatedConfig, { merge: true });
        } catch (err) {
            console.error('Failed to update multi-class assignments:', err);
        }
    };

    const handleRemoveAssignment = async (teacherId, assignmentId) => {
        const newAssignments = (teacherConfigs[teacherId]?.assignments || []).filter(a => a.id !== assignmentId);
            ...
            Object.keys(teacherConfigs).forEach(tId => {
                ...
                (config.assignments || []).forEach(a => {
                    reqs.push({ 
                        ...
                        subject: a.subject, 
                        in: parseInt(config.inPeriod, 10), 
                        out: parseInt(config.outPeriod, 10) 
                    });
                });
            });
    ...
    const handleSaveMasterTimetable = async (e) => {
        ...
        try {
            ...
            for (const teacherId of teacherIds) {
                const docRef = doc(db, 'Data', currentSchoolId, 'timetables', teacherId);
            ...
            const ttSnap = await getDocs(collection(db, 'Data', currentSchoolId, 'timetables'));
            setSavedTimetables(ttSnap.docs.map(docSnap => ({ id: docSnap.id, ...docSnap.data() })));
    ...
    // --- BATCH SUBJECT MAPPING HANDLERS ---
    const handleOpenAssignModal = () => {
        setSelectedClassesForSubjects(['1']);
        setTempAssignedSubjects(classSubjectsMap['1'] || []);
        setNewSubjectInput('');
        setIsAssignSubjectsModalOpen(true);
    };

    const toggleClassForSubjectsModal = (cls) => {
        setSelectedClassesForSubjects(prev => {
            const next = prev.includes(cls) ? prev.filter(c => c !== cls) : [...prev, cls];
            
            // Recalculate merged subjects for the newly selected class group
            const mergedSet = new Set();
            next.forEach(selectedCls => {
                (classSubjectsMap[selectedCls] || []).forEach(sub => mergedSet.add(sub));
            });
            setTempAssignedSubjects(Array.from(mergedSet));
            return next;
        });
    };

    const toggleSelectAllClassesForSubjects = () => {
        if (selectedClassesForSubjects.length === classesList.length) {
            setSelectedClassesForSubjects([]);
            setTempAssignedSubjects([]);
        } else {
            setSelectedClassesForSubjects([...classesList]);
            const mergedSet = new Set();
            classesList.forEach(cls => {
                (classSubjectsMap[cls] || []).forEach(sub => mergedSet.add(sub));
            });
            setTempAssignedSubjects(Array.from(mergedSet));
        }
    };

    // Save subjects across every selected class individually in Firestore: Data -> {schoolId} -> sessions -> {activeSession} -> subjects -> {class}
    const handleSaveAssignedSubjects = async () => {
        if (!currentSchoolId || selectedClassesForSubjects.length === 0) return;
        setIsLoading(true);
        try {
            const updatePromises = selectedClassesForSubjects.map(clsName => {
                const subDocRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'subjects', clsName);
                
                // Merge new subjects with existing class subjects to avoid overwriting unique items
                const existingSubs = classSubjectsMap[clsName] || [];
                const combinedSubs = Array.from(new Set([...existingSubs, ...tempAssignedSubjects]));
                
                return setDoc(subDocRef, { assignedSubjects: combinedSubs }, { merge: true });
            });
            ...
            setClassSubjectsMap(prev => {
                const updated = { ...prev };
                selectedClassesForSubjects.forEach(clsName => {
                    const existingSubs = prev[clsName] || [];
                    updated[clsName] = Array.from(new Set([...existingSubs, ...tempAssignedSubjects]));
                });
                return updated;
            });

            setMessage({ type: 'success', text: `Subjects mapped across ${selectedClassesForSubjects.length} classes!` });
    ...
    const availableSubjectsForSelectedClasses = React.useMemo(() => {
        if (selectedClasses.length === 0) return [];
        const subjectSet = new Set();
        selectedClasses.forEach(cls => {
            (classSubjectsMap[cls] || []).forEach(sub => subjectSet.add(sub));
        });
        return Array.from(subjectSet);
    }, [selectedClasses, classSubjectsMap]);
        ...
        <div className="max-w-7xl mx-auto p-6 md:p-10 bg-white rounded-3xl mt-10 shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-slate-100 font-sans">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 pb-8 border-b-2 border-slate-100">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold text-slate-400 mt-1 flex flex-wrap items-center gap-2 tracking-wide">
                            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
                            <span>Active Session: <span className="text-[#303972]">{formatDisplayValue(activeSession)}</span></span>
                            
                            {/* Super Admin Branch Switcher */}
                            {userRole === 'super' && availableBranches.length > 0 && (
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-slate-200 bg-slate-50 ml-2">
                                    <HiOfficeBuilding className="text-slate-400 text-xs" />
                                    <select 
                                        value={currentSchoolId} 
                                        onChange={(e) => handleBranchChange(e.target.value)} 
                                        className="bg-transparent text-slate-800 font-bold text-xs uppercase outline-none cursor-pointer"
                                    >
                                    ...
                                    onClick={async () => {
                                        if (window.confirm("Are you sure you want to clear all saved timetables?")) {
                                            for (const t of savedTimetables) {
                                                await deleteDoc(doc(db, 'Data', currentSchoolId, 'timetables', t.id));
                                            }
                                            setSavedTimetables([]);
                                        }
                                    }}
                ...
                <div className="bg-[#F8F9FD] p-6 md:p-8 rounded-3xl border border-slate-200 space-y-8 animate-in slide-in-from-bottom-4 duration-300">
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200">
                        <h4 className="text-sm font-black text-[#303972] mb-6 uppercase tracking-widest flex items-center gap-3">
                            ...
                            Staff Constraints & Multi-Class Duty Setup
                        </h4>
                            ...
                            <div className="md:col-span-2">
                                {!selectedConfigTeacher ? (
                                    ...
                                    <div className="space-y-8 animate-in fade-in duration-200">
                                        ...
                                        {/* Period Bounds & Time Inputs */}
                                        <div className="p-6 bg-slate-50 rounded-3xl border border-slate-200 space-y-4">
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                                <div>
                                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5"><HiClock className="text-indigo-600"/> Start Period</label>
                                                    <select
                                                        value={teacherConfigs[selectedConfigTeacher]?.inPeriod || 1}
                                                        onChange={(e) => updateTeacherConfig(selectedConfigTeacher, 'inPeriod', Number(e.target.value))}
                                                        className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-[#303972] outline-none focus:border-indigo-600 transition-colors"
                                                    >
                                                ...
                                                <div>
                                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-1.5"><HiClock className="text-indigo-600"/> End Period</label>
                                                    <select
                                                        value={teacherConfigs[selectedConfigTeacher]?.outPeriod || 8}
                                                        onChange={(e) => updateTeacherConfig(selectedConfigTeacher, 'outPeriod', Number(e.target.value))}
                                                        className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-[#303972] outline-none focus:border-indigo-600 transition-colors"
                                                    >
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 border-t border-slate-200/80">
                                                <div>
                                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Shift Start Time</label>
                                                    <input
                                                        type="text"
                                                        value={teacherConfigs[selectedConfigTeacher]?.startTime || '08:00 AM'}
                                                        onChange={(e) => updateTeacherConfig(selectedConfigTeacher, 'startTime', e.target.value)}
                                                ...
                                                <div>
                                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Shift End Time</label>
                                                    <input
                                                        type="text"
                                                        value={teacherConfigs[selectedConfigTeacher]?.endTime || '01:40 PM'}
                                                        onChange={(e) => updateTeacherConfig(selectedConfigTeacher, 'endTime', e.target.value)}
                                                ...
                                                <div>
                                                    <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Period Interval (Mins)</label>
                                                    <select
                                                        value={teacherConfigs[selectedConfigTeacher]?.periodInterval || 45}
                                                        onChange={(e) => updateTeacherConfig(selectedConfigTeacher, 'periodInterval', Number(e.target.value))}
                                                        className="w-full p-3 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold text-[#303972] outline-none focus:border-indigo-600 transition-colors"
                                                    >
                                        ...
                                        {/* Multi-Class Assignment Form */}
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center">
                                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                                    Assign Duties (Select One or Multiple Classes)
                                                </label>
                                                <button
                                                    type="button"
                                                    onClick={toggleSelectAllClasses}
                                                    className="text-[10px] font-extrabold text-indigo-600 hover:underline uppercase tracking-wider"
                                                >
                                                    {selectedClasses.length === classesList.length ? 'Deselect All' : 'Select All Classes'}
                                                </button>
                                            </div>

                                            {/* Multi-Class Chips Container */}
                                            <div className="flex flex-wrap gap-2 p-3.5 bg-slate-50 border-2 border-slate-200 rounded-2xl max-h-32 overflow-y-auto custom-scrollbar">
                                                {classesList.map((cls) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 border ${
                                                                isSelected 
                                                                    ? 'bg-[#303972] text-white border-[#303972] shadow-sm' 
                                                                    : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'
                                                            }`}
                                                        >
                                                            {isSelected && <HiCheck size={12} className="text-emerald-400" />}
                                                            Class {cls}
                                                        </button>
                                                    );
                                                })}
                                            ...
                                            {/* Subject Dropdown and Add Action */}
                                            <div className="flex flex-col sm:flex-row gap-3">
                                                ...
                                                >
                                                    <option value="">
                                                        {selectedClasses.length === 0 
                                                            ? 'Select at least one class above first...' 
                                                            : 'Select Subject...'}
                                                    </option>
                                                    {availableSubjectsForSelectedClasses.map((s, idx) => (
                                                ...
                                                <button 
                                                    ...
                                                    disabled={selectedClasses.length === 0 || !assignSubject}
                                                ...
                                                >
                                                    Add Layer ({selectedClasses.length})
                                                </button>
                                            </div>

                                            {/* Existing Assigned Duties List */}
                                                    ...
                                                    teacherConfigs[selectedConfigTeacher]?.assignments?.map((a, i) => (
                                                        <div key={a.id} className="flex justify-between items-center p-3.5 bg-white border-2 border-slate-100 rounded-xl shadow-sm group hover:border-indigo-100 transition-colors">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <span className="text-xs font-black text-[#303972] uppercase tracking-wide">
                                                                    Class {a.className} <span className="text-slate-300 mx-2">|</span> <span className="text-indigo-600">{a.subject}</span>
                                                                </span>
                                                            </div>
            ...
            {isAssignSubjectsModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#303972]/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-2xl w-full p-8 space-y-8 animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center pb-5 border-b-2 border-slate-100">
                            <div>
                                <h3 className="text-xl font-black text-[#303972] flex items-center gap-3 uppercase tracking-wider">
                                    <HiBookOpen className="text-indigo-600" size={24} /> Batch Subject Mapping
                                </h3>
                                <p className="text-xs font-semibold text-slate-400 mt-1">Assign subjects to single or multiple classes in one operation</p>
                            </div>
                        ...
                        <div className="space-y-6">
                            {/* Multi-Class Target Selector inside Modal */}
                            <div>
                                <div className="flex justify-between items-center mb-3">
                                    <label className="block text-[11px] font-black text-[#303972] uppercase tracking-widest">
                                        Select Target Classes ({selectedClassesForSubjects.length})
                                    </label>
                                    <button
                                        type="button"
                                        onClick={toggleSelectAllClassesForSubjects}
                                        className="text-[10px] font-extrabold text-indigo-600 hover:underline uppercase tracking-wider"
                                    >
                                        {selectedClassesForSubjects.length === classesList.length ? 'Deselect All' : 'Select All Classes'}
                                    </button>
                                </div>
                                <div className="flex flex-wrap gap-2 p-3.5 bg-slate-50 border-2 border-slate-200 rounded-2xl max-h-36 overflow-y-auto custom-scrollbar">
                                    {classesList.map((cls) => {
                                        const isSelected = selectedClassesForSubjects.includes(cls);
                                        return (
                                            <button
                                                key={cls}
                                                type="button"
                                                onClick={() => toggleClassForSubjectsModal(cls)}
                                                className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 border ${
                                                    isSelected 
                                                        ? 'bg-[#303972] text-white border-[#303972] shadow-sm' 
                                                        : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'
                            ...
                            {/* Add Subject Input */}
                            <div>
                                <label className="block text-[11px] font-black text-[#303972] uppercase tracking-widest mb-3">Add Subject to Selected Classes</label>
                                <div className="flex gap-3">
                                    <input
                                        ...
                                        onKeyDown={(e) => { 
                                            if (e.key === 'Enter') { 
                                                e.preventDefault(); 
                                                if(newSubjectInput.trim() && !tempAssignedSubjects.includes(newSubjectInput.trim())) {
                            ...
                            {/* Active Dictionary */}
                            <div>
                                <label className="block text-[11px] font-black text-[#303972] uppercase tracking-widest mb-3">
                                    Subject Batch List ({tempAssignedSubjects.length})
                                </label>
                                <div className="max-h-52 overflow-y-auto p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl space-y-2.5 custom-scrollbar">
                                    {tempAssignedSubjects.length === 0 ? (
                                        <p className="text-[11px] font-bold text-slate-400 text-center py-6 uppercase tracking-widest">No subjects added to batch list.</p>
                                    ) : (
                        ...
                        <div className="flex justify-end gap-4 pt-6 border-t-2 border-slate-100">
                            ...
                            <button 
                                type="button" 
                                disabled={isLoading || selectedClassesForSubjects.length === 0} 
                                onClick={handleSaveAssignedSubjects} 
                                className="px-8 py-3.5 bg-[#303972] hover:bg-indigo-900 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-lg flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50"
                            >
                                <HiSave size={18} /> Apply to {selectedClassesForSubjects.length} Classes
                            </button>
                        </div>
                    </div>
                </div>
