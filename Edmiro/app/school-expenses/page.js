...
import { 
    ...
    setDoc, Timestamp, deleteDoc, onSnapshot, getDoc 
...
export default function SchoolExpensePortal() {
    ...
    // 1. DYNAMIC SESSION LISTENER: Data -> {schoolId} -> config -> settings
    useEffect(() => {
        ...
        const unsub = onSnapshot(configRef, (snap) => {
            if (snap.exists()) {
                ...
                setAvailableSessions(data.sessions || [ "2024-25" , "2025-26", "2026-27"]);
                setActiveSession(data.activeSession || data.session || "2026-27");
            } else {
                setAvailableSessions(["2025-26", "2026-27"]);
    ...
    // 2. FETCH DATA FROM Data -> {schoolId} -> ...
    const fetchData = useCallback(async (targetSchoolId) => {
        if (!targetSchoolId || !activeSession) return;
        setLoading(true);
        try {
            // Fetch fee payments from Data -> {schoolId} -> sessions -> {activeSession} -> feePayments
            const feeSnap = await getDocs(collection(db, 'Data', targetSchoolId, 'sessions', activeSession, 'feePayments'));
            const feeList = [];
            ...
            // Fetch salary payments from Data -> {schoolId} -> salaryPayments
            const salarySnap = await getDocs(collection(db, 'Data', targetSchoolId, 'salaryPayments'));
            ...
            // Fetch manual account entries from Data -> {schoolId} -> sessions -> {activeSession} -> accounts
            const manualSnap = await getDocs(collection(db, 'Data', targetSchoolId, 'sessions', activeSession, 'accounts'));
    ...
    useEffect(() => {
        if (activeSession && currentSchoolId) {
            fetchData(currentSchoolId);
        }
    }, [currentSchoolId, activeSession, fetchData]);
    ...
    const handleDeleteManualEntry = async (id) => {
        if (!currentSchoolId || !activeSession || !id) return;
        try {
            await deleteDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'accounts', id));
            fetchData(currentSchoolId);
        } catch (err) {
    ...
    const handleSaveManualEntry = async () => {
        if (!amount || !note || !currentSchoolId || !activeSession) return;
        try {
            const manualRef = doc(collection(db, 'Data', currentSchoolId, 'sessions', activeSession, 'accounts'));
            ...
            <div className="max-w-[1440px] mx-auto relative z-10 space-y-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-white p-6 md:p-8 rounded-[28px] shadow-sm border border-slate-100">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto flex-wrap justify-center md:justify-end">
                        ...
                        <button onClick={() => fetchData(currentSchoolId)} className="p-3 bg-slate-50 border border-slate-200 rounded-full text-slate-400 hover:bg-slate-100 transition-all cursor-pointer" title="Refresh Data">
                            <HiRefresh className={`w-5 h-5 ${loading ? 'animate-spin' : ''}`} style={{ color: loading ? colors.primary : undefined }}/>
                        </button>

                        <button onClick={handlePrint} className="flex items-center gap-1.5 px-5 py-3 bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 rounded-full font-bold text-xs transition-all cursor-pointer">
                            <HiPrinter className="w-4 h-4" /> Print
                        </button>

                        <button onClick={handleExportExcel} className="flex items-center gap-1.5 px-5 py-3 bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100 rounded-full font-bold text-xs transition-all cursor-pointer">
                            <HiDownload className="w-4 h-4" /> Excel
                        </button>
                        
                        <button 
                            onClick={() => setIsModalOpen(true)} 
                            className="text-white px-6 py-3 rounded-full font-bold text-xs shadow-md transition-all active:scale-[0.99] cursor-pointer"
                                ...
                                {combinedList.map((t, idx) => (
                                    <tr key={t.id + idx} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="px-8 py-5 text-center flex justify-center items-center">
                                            {t.source === 'manual' ? (
                                                <button 
                                                    onClick={() => handleDeleteManualEntry(t.id)} 
                                                    className="p-2.5 text-slate-300 hover:bg-rose-50 hover:text-rose-500 rounded-full transition-colors cursor-pointer"
                                                    title="Delete Entry"
                                                >
                    ...
                    <div className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-white rounded-[28px] p-8 w-full max-w-md shadow-2xl border border-slate-100 relative">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 p-2.5 rounded-full transition-colors cursor-pointer">
                            ...
                            <div className="flex bg-slate-50 p-1.5 rounded-full mb-6 border border-slate-200">
                                <button 
                                    ...
                                    className={`flex-1 py-2.5 rounded-full font-black text-[10px] uppercase tracking-widest transition-all cursor-pointer ${entryType === 'Expense' ? 'bg-white text-rose-500 shadow-sm border border-slate-100' : 'text-slate-400'}`}
                                >
                                    Expense
                                </button>
                                <button 
                                    onClick={() => setEntryType("Income")} 
                                    className={`flex-1 py-2.5 rounded-full font-black text-[10px] uppercase tracking-widest transition-all cursor-pointer ${entryType === 'Income' ? 'bg-white text-emerald-600 shadow-sm border border-slate-100' : 'text-slate-400'}`}
                                >
                                    Income
                                </button>
                            </div>
                            ...
                            <button 
                                className="w-full py-4 text-white rounded-full font-bold text-xs uppercase tracking-widest shadow-md transition-all active:scale-[0.99] cursor-pointer" 
                                style={{ backgroundColor: colors.primary }}
