...
function AttendancePage() {
    ...
    const fetchStudentsAndAttendance = useCallback(async (targetSchoolId) => {
        if (!selectedClass || !selectedDate || !hasMounted || !activeSession || !targetSchoolId) return;
        
        setLoading(true);
        try {
            // 1. Fetch Students for the Grade: Data -> {schoolId} -> sessions -> {activeSession} -> students
            const studentsRef = collection(db, 'Data', targetSchoolId, 'sessions', activeSession, 'students');
            ...
            // 2. Fetch Attendance Record: Data -> {schoolId} -> sessions -> {activeSession} -> attendance -> {docId}
            const docId = `${selectedDate}_${selectedClass}${showOnlyDummy ? '_dummy' : ''}`;
            const attendanceDocRef = doc(db, 'Data', targetSchoolId, 'sessions', activeSession, 'attendance', docId);
            ...
            // Fill default 'Present' for students not yet saved
    ...
    useEffect(() => {
        fetchStudentsAndAttendance(currentSchoolId);
    }, [currentSchoolId, fetchStudentsAndAttendance]);
    ...
    const handleSaveAttendance = async () => {
        if (students.length === 0 || isSaving || !activeSession || !currentSchoolId) return;
        setIsSaving(true);
        try {
            const docId = `${selectedDate}_${selectedClass}${showOnlyDummy ? '_dummy' : ''}`;
            const attendanceDocRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'attendance', docId);
