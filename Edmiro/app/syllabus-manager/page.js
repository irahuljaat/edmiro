...
import { HiSave, HiX, HiBookOpen, HiCheckCircle, HiClock, HiExclamationCircle, HiPlus, HiTrash, HiChartPie, HiOfficeBuilding } from 'react-icons/hi';
...
function SyllabusManager() {
    ...
    // Fetch active session, subjects, and existing syllabi under Data -> {schoolId} -> ...
    const fetchSystemData = useCallback(async (targetSchoolId) => {
        ...
        try {
            ...
            // 1. Fetch active session from Data -> {schoolId} -> config -> settings
            try {
                const settingsDocRef = doc(db, 'Data', targetSchoolId, 'config', 'settings');
                const settingsSnap = await getDoc(settingsDocRef);
                if (settingsSnap.exists()) {
                    const settingsData = settingsSnap.data();
                    if (settingsData.activeSession || settingsData.session) {
                        currentSession = settingsData.activeSession || settingsData.session;
            ...
            // 2. Fetch assigned subjects per class from Data -> {schoolId} -> sessions -> {currentSession} -> subjects -> {cls}
            const subjectsMap = {};
            ...
            // 3. Fetch all syllabus documents from Data -> {schoolId} -> sessions -> {activeSession} -> syllabus
            const syllabusCollectionRef = collection(db, 'Data', targetSchoolId, 'sessions', currentSession, 'syllabus');
    ...
    // Save syllabus to Data -> {schoolId} -> sessions -> {activeSession} -> syllabus -> {class}_{subject}
    const handleSaveSyllabus = async (e) => {
        e.preventDefault();
        if (!formClass || !formSubject || !currentSchoolId) {
        ...
        try {
            ...
            const docRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'syllabus', docKey);
            ...
            <div className="max-w-[1440px] mx-auto space-y-8 relative z-10">
                ...
                >
                    <div>
                        ...
                        <h2 className="text-2xl font-black tracking-tight flex items-center gap-2 mt-1" style={{ color: colors.text }}>
