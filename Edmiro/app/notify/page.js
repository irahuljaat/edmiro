...
function Toast({ message, type, onClose }) {
    ...
    return (
        <div className={`fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-4 rounded-2xl text-white shadow-2xl text-sm font-semibold max-w-sm animate-slideup ${s.bar}`}>
            ...
            <button onClick={onClose} className="opacity-70 hover:opacity-100 transition ml-2 cursor-pointer"><HiX /></button>
        </div>
    );
}
...
export default function NotifyPage() {
    ...
    const [packageName, setPackageName]     = useState('com.school.mvg'); // App Package Name State
    ...
    // Initial Setup: Retrieve saved user credentials and school branch
    useEffect(() => {
    ...
    // ── Load Data from Data -> {schoolId} -> ... ────────────────────────────────
    const loadSchoolData = useCallback(async (targetSchoolId) => {
        if (!targetSchoolId) return;
        try {
            // 1. Fetch Package Name from Data -> {schoolId} -> config -> schoolDetails
            const schoolDetailsSnap = await getDoc(doc(db, "Data", targetSchoolId, "config", "schoolDetails"));
            if (schoolDetailsSnap.exists()) {
                const details = schoolDetailsSnap.data();
                if (details.packageName || details.appPackageName) {
                    setPackageName(details.packageName || details.appPackageName);
                } else {
                    setPackageName('com.school.mvg');
                }
            } else {
                setPackageName('com.school.mvg');
            }

            // 2. Fetch Active Session from Data -> {schoolId} -> config -> settings
            const cfgDoc = await getDoc(doc(db, "Data", targetSchoolId, "config", "settings"));
            if (!cfgDoc.exists()) return;

            const session = cfgDoc.data().activeSession || cfgDoc.data().session || '2026-27';
            setActiveSession(session);

            // 3. Fetch collections from Data -> {schoolId} -> ...
            const [stuSnap, gradesSnap, teachersSnap, noticesSnap] = await Promise.all([
                getDocs(collection(db, "Data", targetSchoolId, "sessions", session, "students")),
                getDocs(collection(db, "Data", targetSchoolId, "grades")),
                getDocs(collection(db, "Data", targetSchoolId, "teachers")),
                getDocs(collection(db, "Data", targetSchoolId, "sessions", session, "notices")),
            ]);

            const stuList = stuSnap.docs.map(d => ({ id: d.id, ...d.data(), name: d.data().name || d.data().studentName || 'Student' }));
    ...
    const resolveTokens = async () => {
        ...
        if (target === 'all') {
            ...
            const q = query(
                collection(db, "Data", currentSchoolId, "sessions", activeSession, "students"),
    ...
    const cleanStaleTokens = async (staleTokens) => {
        if (!staleTokens?.length || !currentSchoolId) return;
        const snap = await getDocs(collection(db, "Data", currentSchoolId, "sessions", activeSession, "students"));
    ...
    const handleDispatch = async (e) => {
        ...
        if (!activeSession || !currentSchoolId)      return toast$("Session or School context not loaded", "error");
        ...
        try {
            ...
            if (isScheduled) {
                ...
                // Send notification including dynamically fetched packageName
                const apiRes = await fetch('/api/send-notification', {
                    ...
                    body: JSON.stringify({
                        ...
                        packageName: packageName || 'com.school.mvg', // Passed from Data -> {schoolId} -> config -> schoolDetails -> packageName
            ...
            // CustomId for whole school document ID "WholeSchool"
            const customId =
            ...
            // Write notices directly to Data -> {schoolId} -> sessions -> {activeSession} -> notices
            await setDoc(
                doc(db, "Data", currentSchoolId, "sessions", activeSession, "notices", customId),
                {
                    [dateField]: {
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex flex-wrap items-center gap-2 mb-0.5">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border border-slate-200 bg-slate-50 text-indigo-600">
                                        Pkg: {packageName}
                                    </span>
                        ...
                        >
                            ...
                            <div className="px-8 py-6 space-y-6 flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {AUDIENCE.map(({ id, label, Icon }) => (
                                            <button
                                                ...
                                                className={`flex items-center gap-2 px-5 py-3 rounded-full text-[10px] font-black uppercase tracking-wider border transition-all cursor-pointer ${
                                                    target === id
                                ...
                                <div className="min-h-[72px]">
                                    ...
                                    {target === 'teachers' && (
                                        <select
                                            className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-full text-xs font-bold text-slate-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition animate-fadein cursor-pointer"
                                                ...
                                                <div className="absolute z-50 w-full mt-2 bg-white border border-slate-100 rounded-[24px] shadow-xl overflow-hidden">
                                                    {filteredStudents.map(s => (
                                                        <button
                                                            ...
                                                            className="w-full flex items-center justify-between px-6 py-4 hover:bg-slate-50 border-b border-slate-100 last:border-0 transition text-left group cursor-pointer"
                                                        >
                                                            <div>
                                    ...
                                    {target === 'single' && selectedStudent && (
                                        <div className="flex items-center justify-between rounded-[24px] px-6 py-4 text-white shadow-md animate-fadein" style={{ backgroundColor: colors.primary }}>
                                            ...
                                            <button
                                                ...
                                                className="p-2 rounded-full bg-white/10 hover:bg-white/25 transition shrink-0 cursor-pointer"
                                            >
                                        ...
                                        <div className="grid grid-cols-2 gap-3 pt-2 animate-fadein">
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 outline-none focus:border-indigo-400 cursor-pointer"
                                    ...
                                    {!payload.imageUrl ? (
                                        ...
                                        <div className="relative h-36 rounded-[24px] overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center">
                                            ...
                                            <button type="button" onClick={() => setPayload(p => ({ ...p, imageUrl: '' }))}
                                                className="absolute top-3 right-3 p-2 bg-red-500 hover:bg-red-600 text-white rounded-full shadow transition cursor-pointer">
                                                <HiX className="text-xs" />
                                            </button>
                                        </div>
                                    )}
                                ...
                                <button
                                    ...
                                    className="w-full flex items-center justify-center gap-2.5 text-white py-4 rounded-full font-black uppercase tracking-widest text-xs transition-all shadow-md active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
