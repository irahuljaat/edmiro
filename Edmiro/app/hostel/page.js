...
export default function HostelManagementPage() {
  ...
  // ── Fetch Active Session and Data from Data -> {schoolId} -> ... ──
  const fetchAllData = useCallback(async (targetSchoolId) => {
    if (!targetSchoolId) return;
    try {
      setLoading(true);

      // Fetch Active Session from Data -> {schoolId} -> config -> settings
      const settingsRef = doc(db, 'Data', targetSchoolId, 'config', 'settings');
      const settingsSnap = await getDoc(settingsRef);
      
      let currentActiveSession = '2026-27';
      if (settingsSnap.exists()) {
        const data = settingsSnap.data();
        currentActiveSession = data.activeSession || data.session || '2026-27';
      }
      setActiveSession(currentActiveSession);

      // Fetch Hostels from Data -> {schoolId} -> sessions -> {activeSession} -> hotels
      const hotelsRef = collection(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'hotels');
      const hotelsSnap = await getDocs(hotelsRef);
      setHostels(hotelsSnap.docs.map(d => ({ id: d.id, ...d.data() })));

      // Fetch Room Types from Data -> {schoolId} -> sessions -> {activeSession} -> roomTypes
      const roomTypesRef = collection(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'roomTypes');
      const roomTypesSnap = await getDocs(roomTypesRef);
      setRoomTypes(roomTypesSnap.docs.map(d => ({ id: d.id, ...d.data() })));

      // Fetch Hostel Rooms from Data -> {schoolId} -> sessions -> {activeSession} -> hostelRooms
      const roomsRef = collection(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'hostelRooms');
  ...
  const handleSaveRoom = async (e) => {
    ...
    if (!currentSchoolId) { alert('School context missing.'); return; }
    ...
    try {
      if (editingRoomId) {
        const docRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'hostelRooms', editingRoomId);
        await updateDoc(docRef, payload);
        setRooms(rooms.map(r => r.id === editingRoomId ? { id: editingRoomId, ...payload } : r));
        setEditingRoomId(null);
        alert('Hostel Room updated successfully!');
      } else {
        const colRef = collection(db, 'Data', currentSchoolId, 'sessions', activeSession, 'hostelRooms');
  ...
  const handleDeleteRoom = async (id) => {
    if (!window.confirm('Delete this room?') || !currentSchoolId) return;
    try {
      await deleteDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'hostelRooms', id));
  ...
  const handleSaveHostel = async (e) => {
    ...
    try {
      if (editingHostelId) {
        const docRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'hotels', editingHostelId);
        await updateDoc(docRef, payload);
        setHostels(hostels.map(h => h.id === editingHostelId ? { id: editingHostelId, ...payload } : h));
        setEditingHostelId(null);
        alert('Hostel updated successfully!');
      } else {
        const colRef = collection(db, 'Data', currentSchoolId, 'sessions', activeSession, 'hotels');
        const docRef = await addDoc(colRef, payload);
  ...
  const handleDeleteHostel = async (id) => {
    if (!window.confirm('Delete this hostel?') || !currentSchoolId) return;
    try {
      await deleteDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'hotels', id));
  ...
  const handleSaveRoomType = async (e) => {
    ...
    try {
      if (editingTypeId) {
        const docRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'roomTypes', editingTypeId);
        await updateDoc(docRef, payload);
        setRoomTypes(roomTypes.map(t => t.id === editingTypeId ? { id: editingTypeId, ...payload } : t));
        setEditingTypeId(null);
        alert('Room Type updated successfully!');
      } else {
        const colRef = collection(db, 'Data', currentSchoolId, 'sessions', activeSession, 'roomTypes');
        const docRef = await addDoc(colRef, payload);
  ...
  const handleDeleteRoomType = async (id) => {
    if (!window.confirm('Delete this room type?') || !currentSchoolId) return;
    try {
      await deleteDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'roomTypes', id));
