...
export default function BehaviorManagementPage() {
  ...
  const loadData = useCallback(async (targetSchoolId) => {
    ...
    try {
      ...
      let currentActiveSession = '2024-25';
      if (settingsSnap.exists()) {
        const data = settingsSnap.data();
        if (data.activeSession || data.session) {
          currentActiveSession = data.activeSession || data.session;
        }
      }
      setActiveSession(currentActiveSession);

      const sessionPath = `Data/${targetSchoolId}/sessions/${currentActiveSession}`;

      // Fetch all students: Data -> {schoolId} -> sessions -> {activeSession} -> students
      let students = [];
      ...
      // Fetch Behavioral Incidents Collection: Data -> {schoolId} -> sessions -> {activeSession} -> behavioral_incidents
  ...
  // Constant 1-element dependency array
  useEffect(() => {
    loadData(currentSchoolId);
  ...
  const handleSaveIncident = async (e) => {
    e.preventDefault();
    if (!targetStudentId || !currentSchoolId) {
      alert('Please select a student and valid school branch.');
      return;
    }

    try {
      setLoading(true);
      const sessionPath = `Data/${currentSchoolId}/sessions/${activeSession}`;
      const incidentsRef = collection(db, sessionPath, 'behavioral_incidents');
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-800 font-sans">
      ...
      <header className="flex flex-col bg-white border-b border-slate-200 shadow-xs shrink-0 print:hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-8 py-4 gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-0.5">
                <span className="text-xs text-slate-500">Active Session: <span className="font-semibold text-rose-600">{activeSession}</span></span>
