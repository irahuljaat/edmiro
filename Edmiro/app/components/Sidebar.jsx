...
// Master Navigation Item Definitions linked with assignedFeatures mapping
const NAV_CONFIG = [
  { 
    section: 'CORE CENTER', 
    items: [
      { name: 'Dashboard', icon: HiOutlineHome, path: '/dashboard', featureKey: null }, // Always available
      { name: 'Students', icon: HiOutlineUserGroup, path: '/students', featureKey: 'Students Hub' },
      { name: 'Teachers', icon: HiOutlineAcademicCap, path: '/teacher-manage', featureKey: 'Faculty Hub' },
      { name: 'Time Table', icon: HiOutlineCalendar, path: '/time-table', featureKey: 'Schedule Hub' },
      { name: 'Syllabus', icon: HiOutlineDocumentText, path: '/syllabus-manager', featureKey: 'Syllabus Hub' },
      { name: 'Attendance', icon: HiOutlineClipboardList, path: '/attendance', featureKey: 'Attendance' },
      { name: 'Behavioral Records', icon: HiOutlineShieldCheck, path: '/behaviour', featureKey: 'Behavior' },
      { name: 'Vehicle', icon: HiOutlineTruck, path: '/route-management', featureKey: 'Vehicle' },
      { name: 'Enquiries', icon: HiOutlineChatAlt2, path: '/enquiries', featureKey: 'Enquiries' },
      { name: 'Admissions', icon: HiOutlineDocumentText, path: '/Adenquiry', featureKey: 'Admissions' },
      { name: 'Post Maker', icon: HiOutlineSparkles, path: '/posts', featureKey: 'Post Maker' },
      { 
        name: 'Library', 
      ...
      { name: "ID Cards Export", path: "/id-cards", icon: HiOutlineIdentification, featureKey: 'Id Hub' },
      { name: 'Paper Maker', icon: HiOutlinePrinter, path: '/papers', featureKey: 'Class Tests' },
      { name: 'Hostel', icon: HiOutlineOfficeBuilding, path: '/hostel', featureKey: 'Hostel' },
      { name: 'Fee Collection', icon: HiOutlineCurrencyDollar, path: '/fees-system', featureKey: 'Accounts Hub' },
      { name: 'School Expenses', icon: HiOutlineCurrencyDollar, path: '/school-expenses', featureKey: 'Income & Expense' },
      { name: 'Exam Management', icon: HiOutlineClipboardList, path: '/exam-manage', featureKey: 'Reports Hub' },
      { name: 'Notify', icon: HiOutlineBell, path: '/notify', featureKey: 'Notice' },
      { name: 'Settings', icon: HiOutlineCog, path: '/settings', featureKey: null }, // Always available
    ]
  },
];
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200/80 flex flex-col transform transition-transform lg:translate-x-0 lg:static ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {/* Logo Area fetched from Data -> schoolId -> config -> schoolDetails */}
