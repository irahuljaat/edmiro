...
export default function DynamicSchoolMeta({ children }) {
  ...
  useEffect(() => {
    // Retrieve current logged in school ID from localStorage
    ...
    const unsubscribe = onSnapshot(schoolDetailsRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Dynamically update document title in browser tab
        document.title = `${fetchedName} | Educational Management System`;
      } else {
        setSchoolInfo({
          name: 'Edmiro',
          address: 'Smart Educational Management Portal'
        });
        document.title = 'Edmiro | Educational Management System';
      }
    }, (error) => {
