...
const fetchTeacherListWithSalary = async (schoolId) => {
    if (!schoolId) return [];
    try {
...
const fetchPaymentStatus = async (schoolId, monthYear) => {
    if (!schoolId) return {};
    try {
        // Read salary payments from Data -> {schoolId} -> salaryPayments
        const paymentsCollection = collection(db, 'Data', schoolId, 'salaryPayments');
...
const savePaymentRecord = async (schoolId, teacherId, teacherName, monthYear) => {
    if (!schoolId) {
        throw new Error("School context missing.");
    }
    try {
        // Save payment record under Data -> {schoolId} -> salaryPayments -> {teacherId_monthYear}
        const paymentsCollection = collection(db, 'Data', schoolId, 'salaryPayments');
...
function SalaryManagement({ schoolId }) {
    ...
    // Constant 2-element dependency array prevents hook array size mutation
    useEffect(() => {
        let isMounted = true;
        ...
        const loadSalaryData = async () => {
            ...
            try {
                const [teacherList, statusMap] = await Promise.all([
                    fetchTeacherListWithSalary(schoolId),
                    fetchPaymentStatus(schoolId, currentMonthYear)
                ]);
                if (isMounted) {
    ...
    const handleMarkPaid = async (teacherId, teacherName) => {
        setLoading(true);
        try {
            const result = await savePaymentRecord(schoolId, teacherId, teacherName, currentMonthYear);
            if (result.success) {
