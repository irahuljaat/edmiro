...
export default function MarksheetGenerator({ onBack, activeSession: propActiveSession, schoolId = 'TEST_EDMIRO_ACADEMY' }) {
    ...
    const [activeSession, setActiveSession] = useState(propActiveSession || '');
    ...
    // 1. Sync session from Data -> {schoolId} -> config -> settings
    useEffect(() => {
        if (!schoolId) return;
        const unsub = onSnapshot(doc(db, 'Data', schoolId, 'config', 'settings'), (docSnap) => {
            if (docSnap.exists()) {
                const d = docSnap.data();
                setActiveSession(d.activeSession || d.session || propActiveSession || '');
            }
        });
        return () => unsub();
    }, [schoolId, propActiveSession]);

    // 2. Fetch exams from Data -> {schoolId} -> sessions -> {activeSession} -> exams
    useEffect(() => {
        const fetchExams = async () => {
            if (!activeSession || !schoolId) return;
            try {
                const snap = await getDocs(collection(db, 'Data', schoolId, 'sessions', activeSession, 'exams'));
    ...
    // 3. Data fetching aligned with ExamDefinitionAndAssignment
    const fetchData = async () => {
        if (!selectedClass || selectedExams.length === 0 || !activeSession || !schoolId) return;
        setLoading(true);
        setGeneratedData([]);
        setSelectedStudentIds([]);

        try {
            // Students: Data -> {schoolId} -> sessions -> {activeSession} -> students
            let sSnap = await getDocs(query(
                collection(db, 'Data', schoolId, 'sessions', activeSession, 'students'), 
                where('grade', '==', selectedClass)
            ));

            if (sSnap.empty) {
                sSnap = await getDocs(query(
                    collection(db, 'Data', schoolId, 'students'), 
                    where('grade', '==', selectedClass)
                ));
            }

            // Exam Assignments & Marks: Data -> {schoolId} -> sessions -> {activeSession} -> examAssignments/examMarks
            const examData = await Promise.all(selectedExams.map(async (examId) => {
                const docId = `${examId}_${selectedClass}`;
                const mSnap = await getDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'examMarks', docId));
                const aSnap = await getDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'examAssignments', docId));
            ...
            // Filter optional subjects for high school grades
        ...
        <div className="min-h-screen bg-slate-50 p-4 md:p-8 print:p-0 print:bg-white">
            <div className="max-w-6xl mx-auto no-print space-y-6">
                ...
                <div className="flex justify-between items-center">
                    <button 
                        onClick={onBack} 
                        className="text-indigo-600 font-bold uppercase text-[10px] tracking-widest flex items-center gap-2 cursor-pointer"
                    >
                        <HiOutlineChevronLeft/> Back
                    </button>

                    <div className="flex items-center gap-3">
                        {/* Static Component Template Switcher */}
                        <div className="bg-white border border-slate-200 px-3 py-1.5 rounded-xl flex items-center gap-2 shadow-sm">
                            ...
                            >
                                ...
                                <option value="kids">Kids Illustrated Template</option>
                            </select>
                        </div>
                ...
                <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-100 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select 
                                ...
                                className="w-full p-3 bg-slate-50 rounded-xl font-bold border-none ring-1 ring-slate-100 outline-none cursor-pointer"
                            >
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full p-3 bg-slate-50 rounded-xl font-bold ring-1 ring-slate-100 outline-none cursor-pointer" 
                            />
                        </div>
                        ...
                        <div className="flex items-end gap-2">
                            <button 
                                onClick={fetchData} 
                                className="flex-1 bg-indigo-600 text-white p-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition-all cursor-pointer"
                            >
                            ...
                            <button 
                                ...
                                className="bg-slate-800 text-white p-3 rounded-xl disabled:opacity-50 cursor-pointer"
                            >
                                <HiPrinter size={20}/>
                            </button>
                        </div>
                    ...
                    <div>
                        <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2 px-1">
                            3. Select Exams (Order from Left to Right)
                        </label>
                        <div className="flex flex-wrap gap-2 p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                            {exams.map(e => {
                                const orderIdx = selectedExams.indexOf(e.id);
                                const isSel = orderIdx !== -1;
                                return (
                                    <button 
                                        key={e.id} 
                                        onClick={() => toggleExamSelection(e.id)}
                                        className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase transition-all flex items-center gap-2 cursor-pointer ${
                                            isSel ? 'bg-indigo-600 text-white shadow-md scale-105' : 'bg-white text-slate-500 border border-slate-200'
                                        }`}
                                    >
                                        {isSel && (
                                            <span className="bg-white text-indigo-600 w-4 h-4 rounded-full flex items-center justify-center text-[8px]">
                                                {orderIdx + 1}
                                            </span>
                                        )}
                                        {e.name}
                    ...
                    <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                        <div className="p-4 bg-slate-50 border-b flex justify-between items-center">
                            <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                <HiOutlineUserGroup size={16}/> Students: {generatedData.length} | Selected: {selectedStudentIds.length}
                            </h3>
                            <button 
                                onClick={() => setSelectedStudentIds(selectedStudentIds.length === generatedData.length ? [] : generatedData.map(r => r.student.id))} 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 p-4 max-h-60 overflow-y-auto">
                            {generatedData.map(d => {
                                ...
                                return (
                                    <div 
                                        key={d.student.id} 
                                        onClick={() => setSelectedStudentIds(p => p.includes(d.student.id) ? p.filter(i => i !== d.student.id) : [...p, d.student.id])}
                                        className={`p-3 rounded-xl border cursor-pointer transition-all flex items-center justify-between ${
                                            active ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-100'
                                        }`}
                                    >
                                        <span className={`text-[10px] font-bold ${active ? 'text-indigo-700' : 'text-slate-600'}`}>
                                            {d.student.name}
                                        </span>
            ...
            {/* Print Area Rendering Component Templates Directly */}
            <div id="print-area">
