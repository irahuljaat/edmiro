...
function MarksEntry({ activeSession, schoolId = 'TEST_EDMIRO_ACADEMY' }) {
    ...
    const downloadTemplate = () => {
        ...
        const templateData = students.map(s => {
            const row = { "Student ID": s.id, "Student Name": s.name || s.studentName, "Class": selectedClass };
    ...
    const handleFileUpload = (e) => {
        ...
        if (!file || !schoolId || !activeSession) return;
        ...
        reader.onload = async (evt) => {
            try {
                ...
                for (const studentClass in classGroups) {
                    ...
                    const docRef = doc(db, 'Data', schoolId, 'sessions', activeSession, 'examMarks', docId);
    ...
    const loadData = useCallback(async () => {
        if (!activeSession || !selectedExamId || !selectedClass || !schoolId) return;
        setLoading(true);
        try {
            const sSnap = await getDocs(query(collection(db, 'Data', schoolId, 'sessions', activeSession, 'students'), where('grade', '==', selectedClass)));
            ...
            const aSnap = await getDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'examAssignments', assignmentId));

            if (aSnap.exists()) {
                const subs = aSnap.data().subjects || [];
                setAssignedSubjects(subs);

                const mSnap = await getDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'examMarks', assignmentId));
    ...
    }, [activeSession, selectedExamId, selectedClass, showOnlyDummy, schoolId]);

    // Constant array size: [activeSession, schoolId]
    useEffect(() => {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            await setDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'examMarks', `${selectedExamId}_${selectedClass}`), {
                marks: marksData,
                updatedAt: new Date().toISOString()
            }, { merge: true });
            ...
            <div className="relative z-10 space-y-8 animate-in fade-in duration-700">
                ...
                <div className="bg-white rounded-[28px] shadow-sm border border-slate-100 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                        <button onClick={() => router.back()} className="p-3 hover:bg-slate-50 rounded-full transition-colors text-slate-400 cursor-pointer">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex items-center gap-1.5 ml-1">
                            <button onClick={() => fileInputRef.current.click()} className="p-3 bg-white border border-slate-200 text-slate-600 rounded-full hover:bg-slate-50 transition-all shadow-sm cursor-pointer" title="Upload Marks">
                                <HiOutlineUpload size={18} />
                            </button>
                            <input type="file" ref={fileInputRef} hidden accept=".xlsx, .xls" onChange={handleFileUpload} />
                            <button onClick={downloadTemplate} className="p-3 bg-white border border-slate-200 text-slate-600 rounded-full hover:bg-slate-50 transition-all shadow-sm cursor-pointer" title="Download Excel Template">
                        ...
                        <button 
                            onClick={handleSave} 
                            disabled={isSaving} 
                            className="flex items-center gap-2 px-6 py-3 text-white rounded-full font-bold text-xs uppercase tracking-widest shadow-md transition-all active:scale-[0.99] disabled:opacity-50 ml-2 cursor-pointer"
