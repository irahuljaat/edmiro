...
function TimeTableCreator({ activeSession, schoolId = 'TEST_EDMIRO_ACADEMY' }) { 
    ...
    }, [activeSession, schoolId, selectedExamId]);

    const fetchData = useCallback(async (examId, classes) => {
        if (!examId || classes.length === 0 || !activeSession || !schoolId) return;
        setLoading(true);
        setMessage(null);
        try {
            let subjectsMap = new Map();
            for (const cls of classes) {
                const docRef = doc(db, 'Data', schoolId, 'sessions', activeSession, 'examAssignments', `${examId}_${cls}`);
            ...
            const exactSnap = await getDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'timetables', exactId));
            
            let fetchedSchedule = [];
            if (exactSnap.exists()) {
                fetchedSchedule = exactSnap.data().schedule || [];
            } else {
                const q = query(collection(db, 'Data', schoolId, 'sessions', activeSession, 'timetables'), where('examId', '==', examId));
    ...
    const handleSaveTimetable = async () => {
        ...
        try {
            ...
            await setDoc(doc(db, 'Data', schoolId, 'sessions', activeSession, 'timetables', docId), {
            ...
            <div className="relative z-10 space-y-8 animate-in fade-in duration-700">
                ...
                <div className="bg-white rounded-[28px] shadow-sm border border-slate-100 p-6 md:p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                    <div>
                        ...
                        <p className="text-slate-400 font-medium mt-1 text-xs">Configure examination schedules for <span className="font-bold" style={{ color: colors.primary }}>{activeSession}</span></p>
                    </div>
                    <button 
                        onClick={handleSaveTimetable} 
                        disabled={isSaving || schedule.length === 0}
                        className="w-full lg:w-auto text-white px-8 py-3 rounded-full font-bold text-xs uppercase tracking-widest transition-all shadow-md active:scale-[0.99] flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className="bg-slate-900 rounded-[28px] p-6 md:p-8 shadow-xl relative overflow-hidden text-white">
                                    ...
                                    {quickSlots.map((slot) => (
                                        <div key={slot.id} className="relative group">
                                            <button
                                                ...
                                                className={`text-xs px-4 py-2 rounded-full border font-bold transition-all flex items-center gap-1 cursor-pointer ${
                                            ...
                                            {isManagingSlots && (
                                                <button
                                                    ...
                                                    className="absolute -top-2 -right-2 bg-rose-500 text-white rounded-full p-1 text-xs shadow hover:bg-rose-600 cursor-pointer"
                                                >
                                                    <HiOutlineX />
                                                </button>
                                            )}
                                ...
                                {isManagingSlots && (
                                    <form onSubmit={handleAddSlot} className="mt-4 pt-4 border-t border-white/10 grid grid-cols-1 md:grid-cols-4 gap-2">
                                        ...
                                        <input 
                                            ...
                                            className="px-4 py-3 rounded-full bg-white/10 text-white text-xs border border-white/10 outline-none" 
                                        />
                                        <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold uppercase tracking-widest rounded-full py-3 cursor-pointer">
                                            Add Shift Slot
                                        </button>
                                    </form>
                                )}
                            ...
                            <form onSubmit={handleAddScheduleItem} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-10">
                                ...
                                <button type="submit" className="text-white font-black rounded-full transition-all py-3 shadow-lg active:scale-95 text-xs uppercase tracking-widest cursor-pointer" style={{ backgroundColor: colors.primary }}>
                                    Add Item
                                </button>
                            </form>
                        </div>
                            ...
                            {[...schedule].sort((a,b) => new Date(a.date) - new Date(b.date)).map((item) => (
                                <div key={item.subject} className="bg-white p-6 rounded-[28px] border border-slate-100 shadow-sm hover:shadow-md transition-all group">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <button onClick={() => setSchedule(schedule.filter(i => i.subject !== item.subject))} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-rose-500 transition-all p-1 cursor-pointer">
