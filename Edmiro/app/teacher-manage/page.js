...
export default function TeacherManagePage() {
    ...
    // Real-time listener for site configuration active session under Data -> {schoolId} -> config -> settings
    useEffect(() => {
        if (!currentSchoolId) return;

        const configRef = doc(db, 'Data', currentSchoolId, 'config', 'settings');
        const unsubSettings = onSnapshot(configRef, async (snap) => {
            if (snap.exists()) {
                const configData = snap.data();
                setActiveSession(configData.activeSession || configData.session || "2024-25");
            } else {
                // Fallback scan under Data -> {schoolId} -> sessions
                try {
                    const sessionsColRef = collection(db, "Data", currentSchoolId, "sessions");
                    const sessionsSnap = await getDocs(sessionsColRef);
                    const dynamicSessions = sessionsSnap.docs.map(d => d.id);
                    setActiveSession(dynamicSessions[0] || "2024-25");
                } catch (err) {
    ...
    const renderContent = () => {
        switch (currentView) {
            case VIEWS.ADD:
                return <AddTeacherForm schoolId={currentSchoolId} activeSession={activeSession} onSuccess={() => handleViewChange(VIEWS.LIST)} />; 
            case VIEWS.EDIT:
                if (!selectedTeacher) return <ErrorMessage message="Teacher data missing." onReset={() => handleViewChange(VIEWS.LIST)} colors={colors} />;
                return <TeacherEditForm schoolId={currentSchoolId} activeSession={activeSession} teacherData={selectedTeacher} onSuccess={() => {
                    handleViewChange(VIEWS.LIST); 
                }} onCancel={() => handleViewChange(VIEWS.LIST)} />;
            case VIEWS.VIEW_PRINT:
                if (!selectedTeacher) return <ErrorMessage message="Teacher data missing." onReset={() => handleViewChange(VIEWS.LIST)} colors={colors} />;
                return <TeacherViewPrint schoolId={currentSchoolId} teacherData={selectedTeacher} onClose={() => {
                    handleViewChange(VIEWS.LIST); 
                }} />;
            case VIEWS.ATTENDANCE:
                return <TeacherAttendance schoolId={currentSchoolId} activeSession={activeSession} />;
            case VIEWS.SALARY:
                return <SalaryManagement schoolId={currentSchoolId} activeSession={activeSession} />;
            case VIEWS.LIST:
            default:
                return (
                    <TeacherList 
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap items-center gap-3 pt-1">
                            ...
                            {userRole === 'super' && availableBranches.length > 0 && (
                                <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full border border-slate-200 bg-slate-50 shadow-sm">
                                    <HiOutlineOfficeBuilding className="text-slate-400 text-xs" />
                                    <select 
                                        value={currentSchoolId} 
                                        onChange={(e) => handleBranchChange(e.target.value)} 
                                        className="bg-transparent text-slate-800 font-bold text-[10px] uppercase tracking-wider outline-none cursor-pointer"
                                    >
