...
import { collection, query, orderBy, onSnapshot, doc, deleteDoc, getDoc, getDocs } from 'firebase/firestore';
import { 
    HiOutlineUser, 
    HiOutlineCalendar, 
    HiOutlineTrash, 
...
export default function AdmissionsPage({ onTransferToForm }) {
    ...
    // Real-time admissions listener for Data -> {schoolId} -> admissions
    useEffect(() => {
        if (!currentSchoolId) return;

        const admissionsColRef = collection(db, 'Data', currentSchoolId, 'admissions');
        const q = query(admissionsColRef, orderBy('appliedAt', 'desc'));
    ...
    const handleDeleteApplication = async (studentId) => {
        if (!currentSchoolId || !studentId) return;
        if (window.confirm("Are you sure you want to reject and delete this application?")) {
            try {
                await deleteDoc(doc(db, 'Data', currentSchoolId, 'admissions', studentId));
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-3xl rounded-[28px] shadow-2xl border border-slate-100 overflow-hidden relative">
                        ...
                        <div className="p-6 bg-slate-50/80 border-t border-slate-100 flex justify-between items-center">
                            <button 
                                onClick={() => handleDeleteApplication(selectedStudent.id)}
