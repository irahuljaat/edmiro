...
export default function ProfessionalDashboard() {
    ...
    // 1. Session Initialization: Reads Data -> {schoolId} -> config -> settings & Data -> {schoolId} -> sessions
    useEffect(() => {
        ...
        const fetchSessionsAndSettings = async () => {
            try {
                ...
                if (configSnap.exists()) {
                    ...
                    activeSess = configData.activeSession || configData.session;
                }

                if (activeSess && !dynamicSessions.includes(activeSess)) {
                    dynamicSessions.unshift(activeSess);
                }

                if (dynamicSessions.length === 0) {
                    dynamicSessions = ["2024-25", "2025-26"];
                }

                setData({
                    session: activeSess || dynamicSessions[0],
        ...
        const unsubscribe = onSnapshot(configRef, (snap) => {
            if (snap.exists()) {
                const activeSess = snap.data().activeSession;
                if (activeSess) {
                    setData(prev => ({
                        ...prev,
                        session: activeSess,
                        sessions: prev.sessions.includes(activeSess) 
                            ? prev.sessions 
                            : [activeSess, ...prev.sessions]
                    }));
                }
            }
        });
    ...
    // Session Switcher Handler: Updates Data -> {schoolId} -> config -> settings
    const handleSessionChange = async (newSession) => {
        try {
            const configRef = doc(db, "Data", currentSchoolId, "config", "settings");
            await setDoc(configRef, { activeSession: newSession }, { merge: true });
    ...
    // 2. Main Data Fetching using Data -> {currentSchoolId} -> ...
    useEffect(() => {
        if (!data.session || !currentSchoolId) return;
        ...
        const fetchStudentsAndStructures = async () => {
            try {
                const feeStructSnap = await getDocs(collection(db, "Data", currentSchoolId, "sessions", data.session, "studentFeeStructures"));
                const classFeeMap = {};
                feeStructSnap.docs.forEach(docSnap => {
                    classFeeMap[docSnap.id] = Number(docSnap.data().totalFee || 0);
                });

                const studentsSnap = await getDocs(collection(db, "Data", currentSchoolId, "sessions", data.session, "students"));
        ...
        const unsubFees = onSnapshot(collectionGroup(db, 'feePayments'), async (snap) => {
            ...
            const sessionYearMatch = data.session.match(/\d{2,4}/);
        ...
        const attendanceQuery = query(
            collection(db, "Data", currentSchoolId, "sessions", data.session, "attendance"),
            where("date", "==", todayStr)
        );
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        {userRole === 'super' && availableBranches.length > 0 && (
                            <div className="flex items-center gap-2">
                                <HiOutlineOfficeBuilding className="text-gray-500 text-lg" />
                                <span className="text-sm font-semibold text-gray-600">Branch:</span>
                                <select 
                                    value={currentSchoolId} 
                                    onChange={(e) => handleBranchChange(e.target.value)} 
                                    className="bg-white border border-gray-200 text-gray-900 px-4 py-2.5 rounded-full font-bold text-sm cursor-pointer outline-none focus:ring-2 focus:ring-yellow-400 shadow-sm transition-all appearance-none pr-8 relative"
