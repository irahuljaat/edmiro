...
export default function FeesSystemPage() {
    ...
    // 2. DATA FETCHING (FIREBASE Data -> {schoolId} -> ...)
    const fetchFeeData = useCallback(async (targetSchoolId) => {
        if (!targetSchoolId) return;
        setLoading(true);
        try {
            // Fetch Active Session from Data -> {schoolId} -> config -> settings
            const configRef = doc(db, 'Data', targetSchoolId, 'config', 'settings');
            const configSnap = await getDoc(configRef);
            let currentActiveSession = '2026-27';
            if (configSnap.exists()) {
                const cData = configSnap.data();
                currentActiveSession = cData.activeSession || cData.session || '2026-27';
            }
            setActiveSession(currentActiveSession);

            const classStr = selectedClass.toString();

            // Fetch Students from Data -> {schoolId} -> sessions -> {activeSession} -> students
            const sSnap = await getDocs(query(collection(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'students'), where('grade', '==', classStr)));
            setStudents(sSnap.docs.map(d => ({ id: d.id, ...d.data() })));

            // Fetch Fee Structure from Data -> {schoolId} -> sessions -> {activeSession} -> studentFeeStructures -> {class}
            const fSnap = await getDoc(doc(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'studentFeeStructures', classStr));
            if (fSnap.exists()) {
            ...
            // Fetch Payments Ledger from Data -> {schoolId} -> sessions -> {activeSession} -> feePayments
            const pSnap = await getDocs(collection(db, 'Data', targetSchoolId, 'sessions', currentActiveSession, 'feePayments'));
            const paymentMap = {};
            pSnap.forEach(d => {
                const data = d.data();
                paymentMap[d.id] = { 
    ...
    useEffect(() => { 
        fetchFeeData(currentSchoolId); 
    }, [currentSchoolId, fetchFeeData]);
    ...
    const handleUpdateOldDue = async (studentId, currentTotalFee) => {
        ...
        try {
            const studentFeeRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'feePayments', studentId);
    ...
    const handleProcessPayment = async () => {
        if (!selectedStudent || (!paymentAmount && !relaxationAmount) || !currentSchoolId) return;
        setLoading(true);
        try {
            ...
            const studentFeeRef = doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'feePayments', selectedStudent.id);
                ...
                <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                    ...
                    <div className="flex gap-3 items-center flex-wrap">
                        ...
                        <div className="flex bg-white rounded-full shadow-xs border border-slate-200 overflow-hidden items-center px-1">
                            ...
                            <button onClick={() => fetchFeeData(currentSchoolId)} className="p-2.5 hover:bg-slate-50 text-slate-400 rounded-full cursor-pointer transition-colors">
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between">
                            ...
                            <button 
                                onClick={async () => {
                                    ...
                                    try {
                                        await setDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'studentFeeStructures', selectedClass), { totalFee: total, feeBreakdown: feeComponents, updatedAt: serverTimestamp() });
                                        alert("Structure applied!"); fetchFeeData(currentSchoolId); setIsStructureModalOpen(false);
