...
import {
  ...
  ChevronDown, ChevronUp, BarChart2, BookMarked, Tag, Info, KeyRound, Building2
} from 'lucide-react';
...
export default function ExamPipelinePage() {
  ...
  // ── Fetch active session and papers ──────────────────────────────────────────
  const fetchActiveSessionAndPapers = useCallback(async (targetSchoolId) => {
    if (!targetSchoolId) return;
    setLoadingDashboard(true);
    try {
      const settingsSnap = await getDoc(doc(db, 'Data', targetSchoolId, 'config', 'settings'));
      const session = (settingsSnap.exists() && (settingsSnap.data().activeSession || settingsSnap.data().session))
        ? (settingsSnap.data().activeSession || settingsSnap.data().session)
        : activeSession;
        ...
        for (const subj of SUBJECTS) {
          try {
            const papersRef = collection(
              db, 'Data', targetSchoolId, 'sessions', session, 'paper-creation', rootCol, cls, subj, 'papers'
            );
  ...
  }, [activeSession, isOnlineQuiz, filterClass]);

  useEffect(() => {
    fetchActiveSessionAndPapers(currentSchoolId);
  }, [currentSchoolId, isOnlineQuiz, filterClass, fetchActiveSessionAndPapers]);
  ...
  const handleExtractAndSaveSyllabus = async () => {
    ...
    if (!currentSchoolId) return setError('School context missing.');
...
Return ONLY JSON:
{
  ...
  "extractedText": "full raw text..."
      ...
      await setDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'syllabus', docId), {
  ...
  const handleFetchSyllabusFromFirebase = async () => {
    ...
    try {
      const docId = `${selectedClass}_${selectedSubject}`;
      const snap = await getDoc(doc(db, 'Data', currentSchoolId, 'sessions', activeSession, 'syllabus', docId));
      if (snap.exists()) {
...
[{
  ...
  const handleSavePaperToFirestore = async () => {
    if (!currentSchoolId) return alert("School context missing.");
    try {
      const rootCol = isOnlineQuiz ? 'online_quiz' : 'exams';
      const papersRef = collection(
        db, 'Data', currentSchoolId, 'sessions', activeSession, 'paper-creation', rootCol, selectedClass, selectedSubject, 'papers'
      );
      ...
      if (isOnlineQuiz) {
        alert(`✅ Quiz saved!\nPath: Data/${currentSchoolId}/sessions/${activeSession}/paper-creation/online_quiz/${selectedClass}/${selectedSubject}/papers/${docRef.id}\n\nVisible in App: ${quizVisibleInApp ? 'Yes — students can attempt it now' : 'No — hidden until you enable it'}`);
      } else {
        alert(`✅ Exam paper saved!\nPath: Data/${currentSchoolId}/sessions/${activeSession}/paper-creation/exams/${selectedClass}/${selectedSubject}/papers/${docRef.id}`);
      }
  ...
  const toggleQuizVisibility = async (paper) => {
    ...
    try {
      ...
      const paperDocRef = doc(
        db, 'Data', currentSchoolId, 'sessions', session, 'paper-creation', 'online_quiz',
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {userRole === 'super' && availableBranches.length > 0 && (
                <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border border-slate-200 bg-slate-50">
          ...
          <div className="rounded-[28px] border border-slate-100 shadow-sm p-6 md:p-8 bg-white mb-6">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1">Stored under <code className="bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded text-[10px]">Data / {currentSchoolId} / sessions / {activeSession} / paper-creation / {isOnlineQuiz ? 'online_quiz' : 'exams'} / &#123;class&#125; / &#123;subject&#125; / papers</code></p>
              </div>
